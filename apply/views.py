...
from .serializers import applyserilizers_seconde

class applyListApiview(ListAPIView):
    # permission_classes = [permissions.IsAuthenticated]
    queryset = Apply.objects.all()
    serializer_class = applyserilizers_seconde
    
class apply_details_retreive(RetrieveAPIView):
    queryset  = Apply.objects.all()
    serializer_class = applyserilizers_seconde
    lookup_field = 'id'
...
class apply_success_hote_hobe(viewsets.ModelViewSet):
    queryset =Apply.objects.all()
    serializer_class = applyserilizers_seconde
    ...
    def get_queryset(self):
        ...
        job_Id = self.request.query_params.get('job_params')
        if job_Id:
            print("job id",job_Id)
        ...
        if userId:
            ...
            queryset=Apply.objects.filter(user=userId)
